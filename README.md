...
6. Add a new function called updateViews. The function ensures that the PlanetDetailViewController’s planet property, created in step 5, has a planet passed to it.  It does this by unwrapping it, then updating the view controller’s title, UIImageView, and UILabels with the planet’s data.
Please note that this is a widespread design pattern, so be sure to commit it to memory. 
If you have a detail view or table view cell template that displays data, using the ‘updateViews’ method will take the model data passed from the previous view controller’s prepareForSegue function and update the view’s subviews with it.
